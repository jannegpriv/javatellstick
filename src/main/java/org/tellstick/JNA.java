...
public class JNA {
    ...
    public static String nativeLibrary = getNativeLibrary();
    ...
    public interface CLibrary extends Library {
        ...
        CLibrary INSTANCE = Native.loadLibrary(nativeLibrary, CLibrary.class,
                Platform.isWindows()
                        ? Collections.singletonMap(Library.OPTION_CALLING_CONVENTION, Function.ALT_CONVENTION)
        ...
        public int tdSensor(Memory protocol, int protocolLen, Memory model, int modelLen, int[] id, int[] dataTypes);
        // int WINAPI tdSensorValue ( const char * protocol,const char * model,int id,int dataType,char * value,
        // int len,int * timestamp )

        public int tdSensorValue(String protocol, String model, int id, int dataType, Memory value, int len,
                int[] timestamp);
        ...
        public int tdController(int controllerId[], int controllerType[], Memory name, int nameLen, int available[]);
    ...
    private static String getNativeLibrary() {
        if (Platform.isWindows()) {
            ...
            throw new RuntimeException(String.format("Unsupported environment: %s, %s", System.getProperty("os.name"),
